                ...
                <div className="space-y-3">
                    {faqData.map((faq, i) => (
                        <motion.details
                            ...
                            className="group select-none rounded-2xl border border-gray-900/10 bg-white shadow-[0_12px_28px_rgba(2,6,23,0.07)]"
                        >
                            <summary className="flex items-center justify-between p-4 cursor-pointer">
                                <h4 className="font-medium text-gray-900">{faq.question}</h4>
                                <ChevronDownIcon className="h-5 w-5 text-gray-500 group-open:rotate-180 transition-transform" />
                            </summary>
                            <p className="p-4 pt-0 text-sm leading-relaxed text-gray-600">
                                {faq.answer}
                            </p>
                        </motion.details>
                    ))}
